...
import { DOC_MAPPING, FIELDS, SIMPLE_SEARCH_FIELDS } from "./mapping.js";
...
class Storage {
  ...
  async getByUserId(userId) {
    const index = indexFor(LEVELS.private, this.indices);
    try {
      ...
      if (hits.total === 1) {
        const { _id: dinoId = null } = hits.hits[0];
        if (!dinoId) {
          logger.error(`cannot resolve dinoId for ${userId}`);
        }
        return dinoId;
      }
      logger.error(`${hits.total} matches for ${userId}`);
    } catch (e) {
      logger.error(`unable to search for ${userId}: ${e}`);
    }
    return null;
  }
  ...
  async simpleSearch(query, level, _which, after) {
    ...
    try {
      ...
      logger.error(`something went wrong during search ${e.message}`);
